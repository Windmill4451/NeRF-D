...
def render_scene_with_depth(
    ...
    rendered_img, coarse_weights, coarse_t_samples, depths = renderer.render_scene_with_depth(
        default_scene,
        camera,
        pixel_indices,
    ...
    if not fine_scene is None:
        ...
        rendered_img, _, _, depths = renderer.render_scene_with_depth(
            fine_scene,
            camera,
            pixel_indices,
...
def main(cfg: DictConfig) -> None:
    ...
    save_dir = render_dir / "train"
    save_depth_dir = render_dir / 'train_depth'
    save_dir = save_dir / datetime.now().strftime("%Y%m%d-%H%M%S")
    save_dir.mkdir(exist_ok=True, parents=True)
    save_depth_dir = save_depth_dir / datetime.now().strftime("%Y%m%d-%H%M%S")
    save_depth_dir.mkdir(exist_ok=True, parents=True)
    print(
        f"Rendering outputs will be saved under: {str(save_dir)}"
    )

    poses = dataset._poses #dataset._render_poses
    image_fnames = [
    ...
    for view_idx, pose in tqdm(enumerate(poses)):
        ...
        # render
        rendered_img, depths = render_scene_with_depth(
          cfg,
          default_scene,
          fine_scene,
          renderer,
        ...
        torch.save(
            depths,
            str(save_depth_dir / f"r_{int(image_fnames[view_idx])}_depth.pt")
        )
